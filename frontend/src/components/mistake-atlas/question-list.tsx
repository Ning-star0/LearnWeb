...
export function QuestionList({ questions, emptyText = '暂时没有符合条件的错题。' }: { questions: QuestionListItem[]; emptyText?: string }) {
  ...
  return <div className="atlas-card overflow-hidden"><div className="divide-y divide-slate-100">{questions.map((question) => {
    ...
    <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-serif text-lg font-semibold text-slate-950">{reference.primary}</h3><span className="rounded-md bg-slate-900 px-2 py-1 text-[10px] font-semibold text-white">{question.textbook.name}</span>{reference.page ? <span className="text-xs text-slate-400">{reference.page}</span> : null}<StatusPill tone={question.status === 'MASTERED' ? 'green' : question.wrongCount >= 3 ? 'red' : 'blue'}>{question.wrongCount >= 3 && question.status === 'ACTIVE' ? '反复错误' : statusLabel[question.status]}</StatusPill></div><p className="mt-1 truncate text-sm text-slate-600">{question.title}</p><div className="mt-2 flex flex-wrap items-center gap-1.5"><span className="mr-1 text-[11px] text-slate-400">{question.chapter.name}</span>{question.knowledgePoints.slice(0, 3).map(({ knowledgePoint }) => <span key={knowledgePoint.name} className="rounded-md border border-slate-200 bg-white px-2 py-0.5 text-[10px] text-slate-500">{knowledgePoint.name}</span>)}<span className="text-[10px] text-slate-300">编号 {question.code}</span></div></div>
