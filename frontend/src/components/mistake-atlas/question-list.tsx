...
export function QuestionList({ questions, emptyText = '暂时没有符合条件的错题。' }: { questions: QuestionListItem[]; emptyText?: string }) {
  ...
  return <div className="atlas-card overflow-hidden"><div className="divide-y divide-slate-100">{questions.map((question) => {
    ...
    return <Link key={question.id} href={`/questions/${question.id}`} className="group grid gap-4 p-4 transition duration-200 hover:translate-x-1 hover:bg-blue-50/40 sm:grid-cols-[minmax(0,1fr)_160px_90px_24px] sm:items-center">
    <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-serif text-lg font-semibold text-slate-950">{reference.primary}</h3><span className="rounded-md border border-blue-100 bg-blue-50 px-2 py-1 text-[10px] font-semibold text-blue-700">{question.textbook.name}</span>{reference.page ? <span className="text-xs text-slate-400">{reference.page}</span> : null}<StatusPill tone={question.status === 'MASTERED' ? 'green' : question.wrongCount >= 3 ? 'red' : 'blue'}>{question.wrongCount >= 3 && question.status === 'ACTIVE' ? '反复错误' : statusLabel[question.status]}</StatusPill></div><p className="mt-1 truncate text-sm text-slate-600">{question.title}</p><div className="mt-2 flex flex-wrap items-center gap-1.5"><span className="mr-1 text-[11px] text-slate-400">{question.chapter.name}</span>{question.knowledgePoints.slice(0, 3).map(({ knowledgePoint }) => <span key={knowledgePoint.name} className="rounded-md border border-slate-200 bg-white px-2 py-0.5 text-[10px] text-slate-500">{knowledgePoint.name}</span>)}<span className="text-[10px] text-slate-300">编号 {question.code}</span></div></div>
    ...
    <div className="text-xs text-slate-500"><div className="font-medium text-slate-700">下次复习</div><div className="mt-1">{question.nextReviewAt ? question.nextReviewAt.toLocaleString('zh-CN', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : '未安排'}</div></div><ArrowRight className="size-4 text-slate-300 transition group-hover:translate-x-1 group-hover:text-blue-500" />
  </Link>;})}</div></div>;
}
