...
function NavItem({ item, pathname, onNavigate, count }: {
  item: { label: string; href: string; icon: typeof LayoutDashboard };
  pathname: string; onNavigate: () => void; count?: number;
}) {
  const active = item.href === '/' ? pathname === '/' : pathname.startsWith(item.href);
  const Icon = item.icon;
  return (
    <Link href={item.href} onClick={onNavigate} className={`group flex h-10 items-center gap-3 rounded-lg px-3 text-sm transition-colors ${active ? 'bg-[var(--atlas-blue-soft)] font-semibold text-[var(--atlas-blue)]' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-950'}`}>
      <Icon className="size-[18px]" strokeWidth={active ? 2.2 : 1.8} />
      <span className="flex-1">{item.label}</span>
      {count ? <span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${active ? 'bg-white text-[var(--atlas-blue)]' : 'bg-slate-100 text-slate-500'}`}>{count}</span> : null}
    </Link>
  );
}

function Sidebar({ pathname, onNavigate, site, user, subjects, counts }: {
  ...
  return (
    <div className="flex h-full flex-col bg-white">
      <div className="flex h-20 items-center gap-3 border-b border-slate-100 px-6">
        <Image src="/api/site-icon" alt="" width={40} height={40} unoptimized className="size-10 rounded-xl object-cover shadow-sm" />
        <div className="min-w-0"><div className="truncate font-serif text-lg font-semibold tracking-tight text-slate-950">{site.name}</div><div className="truncate text-[11px] font-medium uppercase tracking-[0.14em] text-slate-400">{site.subtitle}</div></div>
      </div>

      <div className="border-b border-slate-100 px-4 py-3">
        <button type="button" onClick={() => setSubjectsOpen(!subjectsOpen)} className="flex w-full items-center gap-3 rounded-xl bg-slate-50 px-3 py-2.5 text-left hover:bg-slate-100">
          <Sigma className="size-4 text-[var(--atlas-blue)]" /><div className="flex-1"><div className="text-xs font-semibold text-slate-800">数学</div><div className="text-[10px] text-emerald-600">完整启用</div></div><ChevronDown className={`size-4 text-slate-400 transition ${subjectsOpen ? 'rotate-180' : ''}`} />
        </button>
        {subjectsOpen ? <div className="mt-2 space-y-1 rounded-xl border border-slate-100 p-2">
      ...
      <nav className="atlas-scroll flex-1 space-y-6 overflow-y-auto px-4 py-5">
        <div className="space-y-1">{mainNavigation.map((item) => <NavItem key={item.href} item={item} pathname={pathname} onNavigate={onNavigate} count={'countKey' in item ? counts[item.countKey!] : undefined} />)}</div>
        <div><div className="mb-2 px-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400">数学知识体系</div><div className="space-y-1">{manageNavigation.map((item) => <NavItem key={item.href} item={item} pathname={pathname} onNavigate={onNavigate} />)}</div></div>
        <div><div className="mb-2 px-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-400">分析与系统</div><div className="space-y-1">{systemNavigation.map((item) => <NavItem key={item.href} item={item} pathname={pathname} onNavigate={onNavigate} />)}</div></div>
      </nav>
      ...
      <div className="border-t border-slate-100 p-4">
        <div className="flex w-full items-center gap-3 rounded-xl p-2">
          ...
          <div className="min-w-0 flex-1"><div className="truncate text-sm font-semibold text-slate-800">{user.username}</div><div className="text-xs text-slate-400">私人空间</div></div>
          <form action={logoutAction}><button title="退出登录" className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700"><LogOut className="size-4" /></button></form>
        </div>
      </div>
    </div>
  );
