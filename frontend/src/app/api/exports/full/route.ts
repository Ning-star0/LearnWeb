...
export async function GET() {
  ...
  const [subjects, questions, memoryCards, siteSettings, learningSettings] = await Promise.all([
    ...
    prisma.memoryCard.findMany({ orderBy: [{ subjectId: 'asc' }, { sortOrder: 'asc' }, { updatedAt: 'desc' }] }),
  ...
  const body = JSON.stringify({ version: 1, exportedAt: new Date().toISOString(), subjects, questions, memoryCards, siteSettings, learningSettings }, null, 2);
  await prisma.auditLog.create({ data: { action: 'FULL_JSON_EXPORTED', entity: 'System', detail: { subjects: subjects.length, questions: questions.length, memoryCards: memoryCards.length } } });
