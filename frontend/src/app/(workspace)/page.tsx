...
export default async function DashboardPage() {
  ...
  return <>
    ...
    <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="今日到期复习" value={String(reviewDue)} note={reviewDue ? '按优先级和到期时间排序' : '今天没有到期任务'} icon={CalendarClock} tone="blue" /><StatCard label="当前未掌握" value={String(total - mastered)} note={`数学错题总数 ${total}`} icon={Target} tone="amber" /><StatCard label="最近 7 天重做" value={String(attemptsThisWeek)} note={`独立正确率 ${accuracy}%`} icon={RotateCcw} tone="slate" /><StatCard label="总体掌握率" value={`${masteryRate}%`} note={`${mastered} / ${total || 0} 道已掌握`} icon={CheckCircle2} tone="green" /></section>
