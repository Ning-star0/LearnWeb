...
import { isViewedToday } from '@/lib/memory-schedule';
...
export default async function MemoryStudyPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-5xl pb-4">
    <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><Link href="/memory" className="atlas-button-secondary"><ArrowLeft className="size-4" />返回手册目录</Link><div className="text-xs font-semibold tabular-nums text-slate-400">第 <span className="text-blue-600">{String(index + 1).padStart(2, '0')}</span> 条 / 共 {cards.length} 条</div></div>
    <header className="mb-6 text-center">
      <div className="mx-auto grid size-11 place-items-center rounded-2xl bg-blue-50 text-blue-600"><Icon className="size-5" /></div>
      <div className="mt-4 flex flex-wrap items-center justify-center gap-2"><StatusPill tone={meta.tone}>{meta.label}</StatusPill><span className="text-xs text-slate-400">{card.category}</span></div>
      <h1 className="mt-3 font-serif text-3xl font-semibold tracking-tight text-slate-950 sm:text-4xl">{card.title}</h1>
      {card.tags.length ? <div className="mt-4 flex flex-wrap justify-center gap-2">{card.tags.map((tag) => <span key={tag} className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[10px] text-slate-500">{tag}</span>)}</div> : null}
    </header>

    <MemoryStudyCard cardId={card.id} content={card.contentMarkdown} summary={card.summary} reviewedToday={isViewedToday(card.lastViewedAt)} />

    <nav aria-label="背诵手册翻页" className="mt-5 grid gap-3 sm:grid-cols-2">
      {previous ? <Link href={`/memory/${previous.id}`} className="atlas-card group flex items-center gap-3 p-4 transition hover:border-blue-200"><ChevronLeft className="size-5 text-slate-300 group-hover:text-blue-500" /><div className="min-w-0"><div className="text-[10px] text-slate-400">上一条 · {String(index).padStart(2, '0')}</div><div className="mt-1 truncate text-sm font-semibold text-slate-700">{previous.title}</div></div></Link> : <div className="atlas-card flex items-center p-4 text-xs text-slate-300"><ChevronLeft className="mr-2 size-5" />已经是第一条</div>}
      {next ? <Link href={`/memory/${next.id}`} className="atlas-card group flex items-center justify-end gap-3 p-4 text-right transition hover:border-blue-200"><div className="min-w-0"><div className="text-[10px] text-slate-400">下一条 · {String(index + 2).padStart(2, '0')}</div><div className="mt-1 truncate text-sm font-semibold text-slate-700">{next.title}</div></div><ChevronRight className="size-5 text-slate-300 group-hover:text-blue-500" /></Link> : <Link href={`/memory/${cards[0].id}`} className="atlas-card group flex items-center justify-end gap-3 p-4 text-right transition hover:border-blue-200"><div><div className="text-[10px] text-slate-400">完成一轮</div><div className="mt-1 text-sm font-semibold text-slate-700">从第一条重新开始</div></div><ArrowRight className="size-5 text-blue-500" /></Link>}
    </nav>
  </div>;
}
