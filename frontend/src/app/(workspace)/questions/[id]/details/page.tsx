      ...
      <article className="space-y-5">
        <div className="atlas-card p-6 sm:p-8">
          <div className="flex flex-wrap gap-2">
            <StatusPill tone={question.materialType === 'EXAMPLE' ? 'amber' : 'slate'}>{question.materialType === 'EXAMPLE' ? '例' : '练习'}</StatusPill>
          ...
          <section className="mt-7"><h2 className="text-sm font-semibold text-slate-900">我的错因</h2><div className="mt-3"><MarkdownContent>{question.wrongReason}</MarkdownContent></div></section>
          {question.reflection ? <section className="mt-6"><h2 className="text-sm font-semibold text-slate-900">复盘总结</h2><div className="mt-3"><MarkdownContent>{question.reflection}</MarkdownContent></div></section> : null}
          {question.reminder ? <section className="mt-6 rounded-xl border-l-4 border-blue-500 bg-blue-50 p-5"><div className="text-xs font-semibold text-blue-700">一句话提醒</div><div className="mt-2"><MarkdownContent>{question.reminder}</MarkdownContent></div></section> : null}
