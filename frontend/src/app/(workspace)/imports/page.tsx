...
export default async function ImportsPage({ searchParams }: { searchParams: Promise<{ completed?: string; rolledBack?: string }> }) {
  ...
  return <>
    <PageHeader eyebrow="System · Backup and restore" title="系统备份与恢复" description="这里仅处理整站完整 JSON。日常学习资料和错题请使用各自的 Markdown 导入入口。" action={<a href="/api/exports/full" className="atlas-button-primary"><Download className="size-4" />导出完整 JSON</a>} />
    <ImportTypeNav active="backup" />
    ...
    <div className="grid gap-5 2xl:grid-cols-[minmax(0,1.35fr)_380px]">
      ...
      <aside className="space-y-5">
        ...
        <div className="rounded-2xl border border-blue-200 bg-blue-50 p-5 text-xs leading-6 text-blue-900"><strong>日常导入不在这里</strong><br /><Link href="/learning-import" className="font-semibold underline underline-offset-2">学习资料导入</Link>用于教材、章节、知识点和公式；<Link href="/questions/import" className="font-semibold underline underline-offset-2">错题导入</Link>用于题目和错因。本页 JSON 只用于整站迁移或灾难恢复。</div>
      </aside>
    </div>
