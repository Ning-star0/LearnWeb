...
interface Question { id: number; type: string; stem: string; book: Book; options: { label: string; content: string }[]; answerJson?: any; }
...
function QuestionsPage() {
  ...
  const startPracticeWithIds = (questionIds: number[]) => {
    const params = new URLSearchParams();
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        <Select value={bookId} onValueChange={(v) => { setBookId(v || ''); setPage(1); }}>
              ...
              <div className="flex items-start gap-3">
                ...
                {user && (
                  <Button
                    ...
                    size="sm"
                    className="shrink-0"
                    onClick={() => startPracticeWithIds([q.id])}
                  >
                    刷这题
                  </Button>
                )}
      ...
      <div className="flex justify-between mt-4">
        ...
        <span className="text-sm text-muted-foreground self-center">共 {total} 题，第 {page} 页</span>
